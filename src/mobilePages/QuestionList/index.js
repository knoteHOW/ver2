...
const QuestionList = () => {
  const [questionTotalCount, setQuestionTotalCount] = useState(12);
  const [questionPage, setQuestionPage] = useState(1);

  const QuestionPageChanged = (pageNumber) => {
  ...
  return (
    <>
      <div className="ql-wrap">
        ...
        <Pagination size="small" current={questionPage} total={questionTotalCount} defaultPageSize={5} onChange={QuestionPageChanged} />
      </div>
      <DetailFooter isPass={true}/>
    </>
  )
}
